...
namespace Quan_ly_nhan_su
{
    public partial class Luongnhanvien : Form
    {
        ...
        private string query_tinhluong= "select nv.MaNV,nv.HoTen N'Họ tên',l.HSLuong,l.HSPhuCap,l.BHYT,l.BHTN,l.BHXH,(l.LuongCB*(l.HSLuong+HSPhuCap))-(l.LuongCB*(l.bhyt+l.BHTN+l.BHXH)) N'Lương thực lãnh'  from dbo.NhanVien nv join Luong l on nv.MaLuong=l.MaLuong and nv.isdelete=1";
        private string query_dsluongnv = "select nv.MaNV,nv.HoTen N'Họ tên',l.HSLuong,l.HSPhuCap,l.BHYT,l.BHTN,l.BHXH,l.LuongCB N'Lương cơ bản'  from dbo.NhanVien nv join Luong l on nv.MaLuong=l.MaLuong and nv.isdelete=1";
        private string ma;
        ...
        private void cb_ma_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            var model = from p in dt.Luongs where p.MaLuong == ma select p;
            foreach(Luong item in model)
            {
                tb_luongcb.Text = item.LuongCB.ToString();
                tb_heso.Text = item.HSLuong.ToString();
                tb_hsphucap.Text = item.HSPhuCap.ToString();
                tb_BHYT.Text = item.BHYT.ToString();
                tb_bhxh.Text = item.BHXH.ToString();
                tb_bhtn.Text = item.BHTN.ToString();
            }
