...
namespace Quan_ly_nhan_su
{
    public partial class DangNhap : Form
    {
        ...
        private void bt_DangNhap_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Bạn chưa nhập tên tài khoản","Error",MessageBoxButtons.OK,MessageBoxIcon.Information);
                ActiveControl = tb_nguoidung;
                return;
            }
            if (tb_matkhau.Text == "")
            {
                MessageBox.Show("Bạn chưa nhập mật khẩu", "Error", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ActiveControl = tb_matkhau;
                return;
            }
            DataSet data = new DataSet();
            string query = "select * from dbo.Account nd where nd.Nameaccount=@taikhoan and nd.Password=@matkhau";
