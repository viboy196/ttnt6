...
namespace Quan_ly_nhan_su
{
    public partial class QuanliNhanvien : Form
    {
        ...
        private int luong;
        string query_allnhanvien = "select nv.HoTen N'Họ và tên',nv.Ngaysinh N'Ngày sinh',nv.GioiTinh N'Giới tính',pb.TenPB N'Phòng Ban',cv.TenCV N'Chức vụ',td.TenTDHV N'Trình độ học vấn',td.ChuyenNganh N'Chuyên ngành',nv.DanToc N'Dân tộc',nv.QueQuan N'Quê quán' from dbo.NhanVien nv,dbo.ChucVu cv, dbo.TrinhDoHV td, dbo.PhongBan pb where nv.MaCV = cv.MaCV and nv.MaTD = td.MaTD and nv.MaPB = pb.MaPB and nv.isdelete=1";
        ...
        private void bt_them_Click(object sender, EventArgs e)
        {
            ...
            {
                if (DataNhanSu.kiemtra("select dbo.testmaNV('" + tb_ma.Text + "')") == false)
                {
                    if (DataNhanSu.kiemtra("select dbo.testPhone('" + tb_sdt.Text + "')") == false)
                    {
                        if (DataNhanSu.kiemtra("select dbo.testContract('" + tb_mahd.Text + "')") == false)
                        {
                            add();
                            MessageBox.Show("Thêm thành công nhân viên:" + cb_hoten.Text, "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            dataGridView1.DataSource = DataNhanSu.Danhsach(query_allnhanvien).Tables[0];
                        }
                        else
        ...
        private void load()
        {
            var model = from p in data.PhongBans where p.active==true select p;
        ...
        {
            if (DataNhanSu.kiemtra("select dbo.testName(N'" + tb_timkiem.Text + "')") == true)
            {
        ...
        private void bt_xoa_Click(object sender, EventArgs e)
        {
                    ...
                    case DialogResult.OK:
                        {
                            ...
                            DataNhanSu.sua("exec dbo.DeleteEmployee N'" + tennv + "'");
                            break;
                        }
            ...
            else MessageBox.Show("Vui lòng nhập tên nhân viên bạn cần xóa!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        ...
        private void cb_nhanvienpb_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            dataGridView1.DataSource = DataNhanSu.Danhsach("select nv.HoTen N'Họ và tên',nv.Ngaysinh N'Ngày sinh',nv.GioiTinh N'Giới tính',pb.TenPB N'Phòng Ban',cv.TenCV N'Chức vụ',td.TenTDHV N'Trình độ học vấn',td.ChuyenNganh N'Chuyên ngành',nv.DanToc N'Dân tộc',nv.QueQuan N'Quê quán' from dbo.NhanVien nv,dbo.ChucVu cv, dbo.TrinhDoHV td, dbo.PhongBan pb where nv.MaCV = cv.MaCV and nv.MaTD = td.MaTD and nv.MaPB = pb.MaPB and nv.active=1 and pb.TenPB=N'"+phongban+"'").Tables[0];
        }
        ...
        private void bt_sua_Click(object sender, EventArgs e)
        {
            if (cb_hoten.Text != "")
            {
                DataNhanSu.sua("exec EditEmployee N'" + tennv + "',N'" + phongban + "',N'" + chucvu + "'," + luong + ",N'" + tb_quequan.Text + "'");
